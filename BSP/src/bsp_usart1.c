...
u8 g_ucaBuff[SENDBUFF_SIZE];
u8 g_ucaRxBuf[SENDBUFF_SIZE]; // 使用全局的数据缓存
u8 g_num = 0;

/**
   * @brief  USART1 TX DMA 配置，外设(USART1->DR)到内存
   * @param  无
   * @retval 无
   */
 void USART1_DMA_Config(void)
 {
     ...
     // 内存地址(要传输的变量的指针)
     DMA_InitStructure.DMA_MemoryBaseAddr = (u32)g_ucaRxBuf;
     // 方向：从外设到内存
